...
export function BrandManager({ initialBrands }: { initialBrands: Brand[] }) {
  ...
  function handleUpdate() {
    ...
    startTransition(async () => {
      ...
      setEditingId(null);
      toast("Marque mise à jour.", "success");
    });
  }
          ...
          <div key={brand.id} className="flex items-center justify-between gap-3 rounded-md border border-border p-4">
            <div className="text-sm">
              <p className="font-medium text-ink">
                {brand.name} <Badge tone={brand.isActive ? "sage" : "neutral"} className="ml-2">{brand.isActive ? "Active" : "Inactive"}</Badge>
              </p>
              <p className="text-xs text-muted">{brand.productCount} produits</p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => {
