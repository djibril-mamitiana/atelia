...
export default async function AdminCustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted">{customer.email} · client depuis le {formatDate(customer.createdAt)}</p>
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-3">
        ...
        <div className="rounded-md border border-border bg-surface p-4">
          ...
          <p className="text-xs text-muted">Adresses</p>
        </div>
      </div>

      <div className="mt-8">
        <h2 className="mb-3 font-medium text-ink">Commandes</h2>
