...
export default async function TutorialPage({ params }: { params: Promise<Params> }) {
  ...
  return (
    <div className="container-page py-10">
      <div className="grid gap-10 lg:grid-cols-[1fr_360px]">
        <div>
          ...
          <div className="mt-5 flex items-center gap-4 text-sm text-muted">
            <span className="flex items-center gap-1.5"><Clock size={15} /> {tutorial.durationMinutes} min</span>
        ...
        {tutorial.products.length > 0 && (
          <div>
            <p className="mb-3 font-medium text-ink">Produits utilisés</p>
      ...
      <div className="mt-10">
        <Link href="/tutoriels" className="text-sm text-muted hover:text-ink">← Tous les tutoriels</Link>
      </div>
    </div>
  );
}
