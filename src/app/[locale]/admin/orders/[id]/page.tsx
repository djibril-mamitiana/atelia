  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted">
        {t("placedOn", { date: formatDate(order.createdAt), name: `${order.user.firstName} ${order.user.lastName}`, email: order.user.email })}
      </p>
            ...
            <div className="flex flex-col divide-y divide-border">
              {order.items.map((item) => (
                <div key={item.id} className="flex items-center justify-between py-2.5 text-sm">
                  ...
                  <span className="text-muted">{t("qtyLine", { qty: item.quantity, price: formatPrice(Number(item.unitPrice)) })}</span>
        ...
        <div className="flex flex-col gap-5">
          <div className="rounded-md border border-border bg-surface p-5">
            ...
            <dl className="flex flex-col gap-2 text-sm">
              ...
              <div className="flex justify-between"><dt className="text-muted">{t("discount")}</dt><dd>-{formatPrice(Number(order.discount))}</dd></div>
              <div className="flex justify-between"><dt className="text-muted">{t("shippingCost")}</dt><dd>{formatPrice(Number(order.shippingCost))}</dd></div>
              <div className="flex justify-between border-t border-border pt-2 font-semibold text-ink"><dt>{t("total")}</dt><dd>{formatPrice(Number(order.total))}</dd></div>
            </dl>
